...
struct SuggestedQuestionsProvider {
    ...
    static func forContact(_ contact: Contact?) -> [SocialBrainMessage] {
        ...
        guard let name = contact?.name, !name.isEmpty else {
            ...
            return [
                SocialBrainMessage(content: "社交备忘录", isFromUser: false, timestamp: Date())
            ]
        }
    ...
    static func forContactInsights(_ contact: Contact?) -> [SocialBrainMessage] {
        guard let name = contact?.name, !name.isEmpty else {
            return [
                SocialBrainMessage(content: "分析一下我与这个联系人的关系现状", isFromUser: false, timestamp: Date()),
                SocialBrainMessage(content: "我们之间有什么共同话题？", isFromUser: false, timestamp: Date()),
                SocialBrainMessage(content: "如何改善我们的关系？", isFromUser: false, timestamp: Date())
            ]
        }
        
        return [
            SocialBrainMessage(content: "分析一下我与 \(name) 的关系现状", isFromUser: false, timestamp: Date()),
            SocialBrainMessage(content: "我与 \(name) 之间有什么共同话题？", isFromUser: false, timestamp: Date()),
            SocialBrainMessage(content: "如何改善我与 \(name) 的关系？", isFromUser: false, timestamp: Date()),
            SocialBrainMessage(content: "\(name) 最近有什么变化值得关注？", isFromUser: false, timestamp: Date())
        ]
    }

    static func general() -> [SocialBrainMessage] {
        return [
            SocialBrainMessage(content: "怎样使用 社交大脑", isFromUser: false, timestamp: Date())
        ]
    }
} 